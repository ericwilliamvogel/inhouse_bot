...
namespace bot_2
    {
        ...
        public class DotaClient
        {
            ...
            public async Task Connect(string userName, string password)
            {
                ...
                _callbackManager.Subscribe<SteamUser.LoggedOffCallback>(EventOnLoggedOff);
                ...
                _callbackManager.Subscribe<SteamUser.UpdateMachineAuthCallback>(EventOnDenied);

                while (!await LoadServersList())
                {
                    Thread.Sleep(1000);
                }
            ...
            private async Task<bool> LoadServersList()
            {
            var loadServersTask = await SteamDirectory.LoadAsync(SteamConfiguration.Create(
                (ISteamConfigurationBuilder builder) =>
                {
                    builder.WithCellID(0u);
                })
                );
            //loadServersTask.Wait();

            if (loadServersTask == null)
            {
                Console.WriteLine("servers null");
                return false;
            }

            if (loadServersTask.Count <= 0)
            {
                Console.WriteLine("servers 0");
                return false;
            }

            Console.WriteLine("servers = " + loadServersTask.Count);
            return true;
            }
            ...
            private void EventOnConnected(SteamClient.ConnectedCallback callback)
            {
            ...
            Console.WriteLine(callback.Result);
                ...
                {
                    Console.WriteLine("log failed, steamguard");
            ...
            private void EventOnLoggedOff(SteamUser.LoggedOffCallback callback)
            {

            }

            private void EventOnDenied(SteamUser.UpdateMachineAuthCallback callback)
            {
                _logger.Info(String.Format("denied"));
            }
